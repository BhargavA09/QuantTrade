...
export const fetchMarketOverview = async () => {
  try {
    const prompt = `Provide an overview of major global markets. Include 5 top assets for each category:
    1. US Markets (NASDAQ/NYSE)
    2. Canadian Markets (TSX)
    3. European Markets (LSE/Euronext)
    4. Asian Markets (Nikkei/Hang Seng)
    5. Cryptocurrencies
    6. Commodities (Gold, Oil, etc.)
    
    For each asset, include:
    - ticker: string (e.g. "AAPL", "RY.TO", "BTC-USD", "GC=F")
    - name: string (company/asset name)
    - sector: string
    - marketCap: string (or volume for commodities)
    - recentPerformance: number (percentage change in last 30 days)
    
    Return ONLY JSON:
    {
      "us": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }],
      "canada": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }],
      "europe": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }],
      "asia": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }],
      "crypto": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }],
      "commodities": [{ "ticker": string, "name": string, "sector": string, "marketCap": string, "recentPerformance": number }]
    }`;
    ...
    return {
      ...
      canada: Array.isArray(parsed.canada) ? parsed.canada : [],
      europe: Array.isArray(parsed.europe) ? parsed.europe : [],
      asia: Array.isArray(parsed.asia) ? parsed.asia : [],
    ...
    return { us: [], canada: [], europe: [], asia: [], crypto: [], commodities: [] };
  }
};
