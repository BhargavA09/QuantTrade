...
import { Calculator, DollarSign, TrendingUp, TrendingDown, Info, BarChart3, Activity, Loader2 } from 'lucide-react';
import { StockData } from '../types';
import { fetchFairValue } from '../services/api';
...
const FairValueAnalysis: React.FC<FairValueAnalysisProps> = ({ data }) => {
  ...
  const [fairValueData, setFairValueData] = useState<any>(null);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const loadFairValue = async () => {
      setLoading(true);
      const result = await fetchFairValue(data.ticker);
      if (result) {
        setFairValueData(result);
        setCustomGrowthRate(result.inputs?.growthRate || 5);
        setCustomEps(result.inputs?.eps || (currentPrice / 15));
      }
      setLoading(false);
    };
    loadFairValue();
  ...
  const dcfValue = fairValueData?.dcfValue || (currentPrice * 1.15); // Discounted Cash Flow
  const grahamNumber = fairValueData?.grahamNumber || (currentPrice * 0.92); // Graham Number
  const multiplesValue = fairValueData?.multiplesValue || (currentPrice * 1.08); // Comparable Multiples
  ...
  return (
    <div className="space-y-6 relative">
      {loading && (
        <div className="absolute inset-0 z-10 bg-zinc-950/50 backdrop-blur-sm flex items-center justify-center rounded-3xl">
